...
export default function HowWeWorkPage() {
  const [items, setItems] = useState<HowWeWork[]>([]);
  const [image, setImage] = useState<File | string | null>(null);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const { register, handleSubmit, reset, setValue } = useForm<HowWeWorkForm>();
  ...
  const onSubmit = async (data: HowWeWorkForm) => {
    try {
      ...
      if (editingId) {
        await api.put(`/how-we-work/${editingId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Успешно обновлено");
      } else {
        await api.post("/how-we-work", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      await api.delete(`/how-we-work/${id}`);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Как мы работаем</h1>
        <button
          onClick={() => {
            reset();
            setEditingId(null);
